...
public:
    ...
    // Live INPUT monitor (see PluginProcessor::processBlock's idle tail): while genuinely idle
    // (not recording/playing/scrubbing), fed with the same block peak min/max shape the
    // recording scope uses, from the untouched input -- the host's input in VST/AU, whatever's
    // selected in Audio Settings in Standalone -- so WaveformDisplay can show a live oscilloscope
    // of what's arriving. A SEPARATE ring from scopeMin/scopeMax on purpose, even though the two
    // states are mutually exclusive -- keeps this feature fully additive and the recording path
    // untouched.
